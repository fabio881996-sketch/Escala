...
# 2. CSS - FOCO TOTAL NA COR BRANCA DOS SUBTÍTULOS
st.markdown("""
    <style>
    /* FUNDO DA PÁGINA */
    ...
    /* O FUNDO DO CABEÇALHO PARA O BRANCO SE LER */
    .streamlit-expanderHeader {
        background-color: #455A64 !important;
        border-radius: 10px 10px 0 0 !important;
    }

    /* --- ESTA É A CORREÇÃO QUE PEDISTE --- */
    /* Força a cor BRANCA em todos os elementos de texto do cabeçalho */
        ...
        if df_dia is not None:
            ...
            if not meu_df.empty:
                st.markdown(f"""<div style="background: #FFFFFF; padding: 25px; border-radius: 12px; border: 1px solid #EAECEF; border-left: 5px solid #455A64;">
                    <h1 style="margin:0; color: #1A1C1E !important;">{meu_df.iloc[0]['serviço']}</h1>
        ...
        if df_dia is not None:
            ...
            def filtrar_e_mostrar(titulo, keywords):
                ...
                if not temp_df.empty:
                    # ESTE É O TITULO QUE FICARÁ BRANCO
