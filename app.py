...
# --- 3. FUNÇÃO DO PDF DA ESCALA (GERAÇÃO OFICIAL) ---
def gerar_pdf_escala_dia(data_str, df_original):
    ...
    # 3. PATRULHAS (Ordenação por PO -> Resto -> Horário)
    pdf.set_font("Arial", "B", 8)
    ...
    if not df_pat.empty:
        ...
        def prioridade_servico(nome):
            n = str(nome).lower()
            if 'po' in n or 'ocorrências' in n: return "0_PO"
            return "1_" + n
    ...
    if not df_remu.empty:
        pdf.ln(4)
        pdf.set_font("Arial", "B", 8)
        pdf.cell(0, 6, clean(" SERVIÇOS REMUNERADOS"), 1, 1, 'L', True)
        pdf.set_font("Arial", "B", 7)
...
if not st.session_state["logged_in"]:
    ...
    if menu == "📅 Minha Escala":
        ...
        if not df_dia.empty:
            df_at_v = df_dia.copy()
            df_at_v['id_disp'] = df_at_v['id'].astype(str)
            ...
            def mostrar_sec_geral(tit, keys, df_f, mostrar_extras=False):
                ...
                if not temp.empty:
                    with st.expander(f"🔹 {tit.upper()}", expanded=True):
                        ...
                        if mostrar_extras:
                            ag = temp.groupby(cols_ag, sort=False).agg({'id_disp': lambda x: ', '.join(x), 'viatura': lambda x: ', '.join(x.unique()), 'indicativo rádio': lambda x: ', '.join(x.unique()), 'observações': lambda x: ', '.join(x.unique())}).reset_index()
                        else:
            ...
            df_res = df_at_v.copy()
            df_aus_v = df_res[df_res['serviço'].str.lower().str.contains("férias|licença|doente|diligência|tribunal", na=False)].copy()
            ...
            if not df_outros_v.empty: mostrar_sec_geral("Outros Serviços", [""], df_outros_v, False)
            if not df_remu_v.empty: mostrar_sec_geral("Remunerados", ["remu", "grat"], df_remu_v, True)
            if not df_folga_v.empty: mostrar_sec_geral("Folga", ["folga"], df_folga_v, False)
            
            if not df_aus_v.empty:
                with st.expander("🔹 AUSENTES", expanded=True):
                    ag = df_aus_v.groupby(['serviço', 'horário'], sort=False)['id_disp'].apply(lambda x: ', '.join(x)).reset_index()
        ...
        if m.empty:
            st.write("Não tens pedidos pendentes.")
        for idx, r in m.iterrows():
            st.info(f"{r['data']}: ID {r['id_origem']} quer trocar o serviço dele ({r['servico_origem']}) pelo teu ({r['servico_destino']}).")
            if st.button("✅ ACEITAR PEDIDO", key=f"a{idx}"):
                atualizar_status_gsheet(idx, "Pendente_Admin")
                st.success("Aceite! Aguarda validação do Comando.")
                st.rerun()

    elif menu == "⚖️ Validar Trocas":
        st.title("⚖️ Validação de Trocas (Comando)")
        pendentes = df_trocas[df_trocas['status'] == 'Pendente_Admin']
        if pendentes.empty:
            st.write("Nada para validar.")
        for idx, r in pendentes.iterrows():
            st.warning(f"Troca {r['data']}: {r['id_origem']} ↔️ {r['id_destino']}")
            if st.button("✔️ APROVAR TROCA", key=f"v{idx}"):
