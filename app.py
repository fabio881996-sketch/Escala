...
def gerar_pdf_troca(dados: dict) -> bytes:
    ...
    # Cabeçalho: imagem à esquerda, título à direita
    ...
    try:
        ...
        cab_w = 95*mm
        cab_h = cab_w * (235/398)
        c.drawImage(_cab_path, 20*mm, h - 8*mm - cab_h, width=cab_w, height=cab_h, preserveAspectRatio=True)
        _os.unlink(_cab_path)
        titulo_x = 20*mm + cab_w + 10*mm
        titulo_y = h - 8*mm - cab_h/2
        c.setFillColor(HexColor('#000000'))
        c.setFont(fn_bold, 15)
        c.drawString(titulo_x, titulo_y, "TROCA DE SERVIÇO")
        y_inicio = h - 8*mm - cab_h - 10*mm
    except Exception:
        c.setFillColor(HexColor('#000000'))
        c.setFont(fn_bold, 14)
        c.drawCentredString(w/2, h - 20*mm, "TROCA DE SERVIÇO")
        y_inicio = h - 35*mm

    # Corpo
    style = ParagraphStyle('body', fontName=fn, fontSize=11, leading=18, spaceAfter=6)
    texto = (
        f"O militar <b>{s(dados['nome_origem'])}</b> (ID {s(dados['id_origem'])}), solicitou a autorização "
        f"para a troca do serviço <b>'{s(dados['serv_orig'])}'</b> pelo serviço <b>'{s(dados['serv_dest'])}'</b> "
        f"do militar <b>{s(dados['nome_destino'])}</b> (ID {s(dados['id_destino'])}), para o dia "
        f"<b>{s(dados['data'])}</b>. O pedido foi aceite pelo militar de destino e autorizado superiormente "
        f"no dia <b>{s(dados['data_val'])}</b> pelo <b>{s(dados['validador'])}</b>."
    )
    p = Paragraph(texto, style)
    pw, ph = p.wrap(170*mm, h)
    p.drawOn(c, 20*mm, y_inicio - ph)
